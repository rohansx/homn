...
- **Observation**: The normalized unit of captured activity after it passes the gate. Attributes: id, source kind (screen OCR / a11y tree / ambient audio / dictation / later account connectors), app, capture time (valid-time start), post-redaction text, redaction references, optional session id, optional speaker tag, content hash. Never stores unredacted text.
...
- **Session**: A first-class grouping of observations (a meeting or work-focus block) with boundaries, enabling episode-level consolidation.
...
- **Out of scope for this spec**: The proactive live-meeting copilot (predictive retrieval + local triage + whispered suggestions) and the cursor-buddy/voice body. These are Phases 4–5 and get their own spec after launch signal.
